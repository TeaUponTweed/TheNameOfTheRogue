...
/// Some components
pub type Components<S> = VecMap<S>;
	...
	(name: $name:ident,
		...
		impl World for $name {
			fn new() -> $name {
				...
				$name {
					count: 0,
					free_spots: Vec::with_capacity(50),
					$($cfield: VecMap::new(),)+
			...
			fn update(&mut self, dt: f64) {
				// use std::iter;
				// use std::mem;
				let entities:Vec<Entity> = (0..self.count)
					.filter(|id| !self.free_spots.iter().any(|oid| oid == id))
					.collect();
				$(
					self.$pfield.run(dt, entities.as_slice(), $(&mut self.$cfield2),+);//$(unsafe { mem::transmute(&self.$cfield2) }),+);
				)+
			}
		}
